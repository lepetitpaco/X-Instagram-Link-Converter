#!/bin/bash
# Script pour créer le package Firefox (Manifest V2) prêt à publier
...
echo "📦 Création du package Firefox (Manifest V2)..."

PACKAGE_NAME="x-instagram-link-converter-firefox"
TEMP_DIR=".firefox-package"
...
if [[ "$ZIP_FILE" == *.tar.gz ]]; then
  ...
  echo "⚠️  Note: C'est un .tar.gz, vous devrez le convertir en .zip pour Firefox"
  echo "    Commande: tar -xzf $ZIP_FILE && zip -r ${ZIP_FILE%.tar.gz}.zip * && rm -rf $ZIP_FILE"
else
  echo "✅ Package créé : $ZIP_FILE"
  echo "📦 Prêt à être uploadé sur Firefox Add-ons"
